...
if [ "$1" == "base" ] ; then
    ...
    docker build --no-cache -t canal/osbase $BASE/base_v2 -f $BASE/base_v2/Dockerfile_v2
elif [ "$1" == "base_admin" ] ; then
    docker build --no-cache -t canal/osadmin_admin $BASE/base_admin -f $BASE/base_admin/Dockerfile_admin
elif [ "$1" == "admin" ] ; then
